...
namespace ControlGastos.Controllers
{
    public class TransaccionesController: Controller
    {
        ...
        {
            var usuarioId = servicioUsuarios.ObtenerUsuarioId();
            IEnumerable<ResultadoObtnerPorSemana> transaccionesPorSemana = await servicioReportes.ObtenerReporteSemanal(
                usuarioId, mes, año, ViewBag);

            var agrupado = transaccionesPorSemana.GroupBy(x => x.Semana).Select(x => 
            new ResultadoObtnerPorSemana()
            {
                Semana= x.Key,
            ...
            for (int i = 0; i < diasSegmentados.Count(); i++)
            {
                ...
                {
                    agrupado.Add(new ResultadoObtnerPorSemana()
                    {
                        Semana= semana,
        ...
        public async Task<IActionResult> Mensual(int año)
        {
            ...
            var transaccionAgrupadas = transaccionesPorMes.GroupBy(x => x.Mes).
            ...
            {
                var transaccion = transaccionAgrupadas.FirstOrDefault(x => x.Mes == mes);
                var fechaReferancia = new DateTime(año, mes, 1);
                if (transaccion is null)
                {
                    transaccionAgrupadas.Add(new ResultadoObtenerPorMes()
                    {
                        Mes = mes,
                        FechaReferencia = fechaReferancia
                    });
                }
                else
                {
                    transaccion.FechaReferencia = fechaReferancia;
                }
            }
                transaccionAgrupadas = transaccionAgrupadas.OrderByDescending(x => x.Mes).ToList();

                var modelo = new ReporteMensualViewModel();
                modelo.Año = año;
                modelo.TransaccionesPorMes = transaccionAgrupadas;
        ...
        public async Task<JsonResult> ObtenerTransaccionesCalendario(DateTime start, DateTime end)
        {
            ...
            var eventosCalendario = transacciones.Select(transaccion => new EventoCalendario()
            {
                ...
                End = transaccion.FechaTransaccion.ToString("yyyy_MM-dd"),
